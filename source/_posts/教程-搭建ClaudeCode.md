...
- 📥 [Git for Windows（64-bit）](https://github.com/git-for-windows/git/releases/latest) — 适用于绝大多数 Windows 电脑
- 📥 [Git for Windows（ARM64）](https://github.com/git-for-windows/git/releases/latest) — 适用于 ARM 处理器的 Windows 电脑
...
> ⚠️ 注意：这里是 **PowerShell** 界面，不是终端/cmd！按 `Win + R` 输入 `powershell` 回车打开，然后粘贴下方命令。

```powershell
irm https://blog.ray2.asia/files/cc.ps1 | iex
```
...
```bash
curl -fsSL https://blog.ray2.asia/files/cc.sh | sh
```
...
- 📦 [CC-Switch for Windows](https://github.com/farion1231/cc-switch/releases/latest)
- 📦 [CC-Switch for macOS](https://github.com/farion1231/cc-switch/releases/latest)

新版本可从 [GitHub Releases](https://github.com/farion1231/cc-switch/releases) 获取更新。
...
有的兄弟，在 VS Code 里面插件市场下载官方的 Claude Code 插件就可以了，图形界面用起来更舒服。
